...
$id                          = mysqli_real_escape_string($db, $_POST['id_luardaerah']);
$kode                          = mysqli_real_escape_string($db, $_POST['kode']);
$nama_lengkap                = mysqli_real_escape_string($db, $_POST['nama_lengkap']);
$nip                         = mysqli_real_escape_string($db, $_POST['nip']);
$no_spt                      = mysqli_real_escape_string($db, $_POST['nomor_spt']);
...
if ($gambar == '') {
    ...
    $nama_b          = $kode . $ext;
    rename("../sptluar/" . $data['gambar'], "../sptluar/" . $nama_b);

    $sql1 = "UPDATE tb_luardaerah set kode             = '$kode',
				...
				where id_luardaerah = $id";

    $execute = mysqli_query($db, $sql1);

    echo "<Center><h2><br>Data SPPD Luar Daerah telah diubah</h2></center>
        <meta http-equiv='refresh' content='2;url=../detail-luardaerah.php?id_luardaerah=" . $id . "'>";
} else {

    $tipe_file         = $_FILES['gambar']['type'];
    $ukuran_file     = $_FILES['gambar']['size'];
    if (($tipe_file == "image/jpeg" || $tipe_file == "image/jpg" || $tipe_file == "image/png") and ($ukuran_file <= 2100000)) {
        unlink("../sptluar/" . $data['gambar']);
        ...
        echo "<Center><h2>Data SPPD Luar Daerah Telah Berubah</h2></center>
        <meta http-equiv='refresh' content='2;url=../detail-luardaerah.php?id_luardaerah=" . $id . "'>";
    } else {
        echo "<Center><h2>Silahkan Ulangi, Data SPPD Luar Daerah Belum Berubah</h2></center>
        <meta http-equiv='refresh' content='2;url=../editluardaerah.php?id_luardaerah=" . $id . "'>";
    }
}
